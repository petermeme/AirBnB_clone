...
class TestCity(TestCase):
    ...
    def test_str(self):
        """Tests the verbose representation of a City"""
        self.assertEqual(str(self.model),
                         f"[City] ({self.model.id})"
                         f" {self.model.__dict__}")

    def test_save(self):
        """Test whether City object is successfully saved when save() is
        called"""
        model = City()
        model.save()
    ...
    def test_dict_content(self):
        ...
        self.assertEqual(self.model.updated_at.isoformat(),
                         vals.get('updated_at'))
        self.assertEqual('City', vals.get('__class__'))
